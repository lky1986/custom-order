...
(function($, window, document) {
    ...
    ColResize.prototype = {
        ...
        "_fnSetElementCursor": function(element, bForceResizeMode){
            if (!element) return;
            var cursorStyle = (bForceResizeMode === undefined || bForceResizeMode === false) ? "pointer" : "ew-resize";
        ...
        "_fnMouseDown": function ( e, nTh )
        {
            ...
            $(document).bind( 'mouseup.ColResize', function (e) {
                ...
                //that.oTable.DataTable.settings.bAjaxDataGet = false;
                that._fnSetColumnWidth(nTh);
                //that.oTable.DataTable.settings.bAjaxDataGet = true;
                if (nTh.index()!== that.nLastTh.index()) {
                    that._fnSetColumnWidth(that.nLastTh);
                }
                // http://127.0.0.1:8080/jwms-web/configuration/systemCode/listSystemCode
        ...
        "_fnSetColumnWidth" : function(nTh){
            ...
            $(rows).each(function(i, item) {
                $(item.nTr).find('td').eq(iColumnIndex).width(nTh.width());
            });
