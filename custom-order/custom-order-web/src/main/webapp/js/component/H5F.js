...
(function (root, factory) {
    ...
    var d = document,
        ...
        urlPatt = /[a-z][\-\.+a-z]*:\/\//i,
//        numberPatt = /([0-9]\d*\.?\d*)|(0\.\d*[0-9])/,
        numberPatt = /([0-9]\d*\.?\d*)|(0\.\d*[0-9])|(-[0-9]\d*\.?\d*)|(-0\.\d*[0-9])/,
...
//            missing = required(elem),
        ...
        elem.validity = {
            ...
            rangeOverflow: max,
            stepMismatch: step,
            decimalmatch:decimal,
            customError: customError,
            valid: (!missing && !patt && !step && !decimal && !min && !max && !customError)
        };
//        alert(!missing && !patt && !step && !decimal && !min && !max && !customError);
    ...
    checkField = function(e) {
        ...
        if(nodes.test(el.nodeName) && !(ignoredTypes.test(el.type) || ignoredTypes.test(el.nodeName))) {
            ...
            if(el.validity.valid && (el.value !== "" || specialTypes.test(el.type)) || /*(el.value !== el.getAttribute("placeholder") &&*/ el.validity.valid/*)*/) {
                ...
                }else if(!el.validity.valid||el.validity.patternMismatch) {
    ...
    checkValidity = function(el) {
        var f, ff, isRequired, hasPattern,hasDecimal, invalid = false;
        ...
        if(el.nodeName.toLowerCase() === "form") {
            ...
            for(var i = 0,len = f.length;i < len;i++) {
                ...
                hasDecimal = !!(ff.attributes["decimal"]);
...
//					}
//				}
                if(ff.nodeName.toLowerCase() !== "fieldset") {
                    if (isRequired||hasPattern && !isRequired||hasDecimal){
                        checkField(ff);
                        if(isRequired){
                            triggerInvalid(ff);
                        }
...
//            alert("节点："+el.nodeName.toLowerCase()+"\n invalid :  "+invalid);
            return !invalid;
        } else {
            checkField(el);
            triggerInvalid(el);
    ...
    decimalCheck = function(el){
        var val = el.value;
        if(val!==""){
            var decimal = el.getAttribute("decimal");
            //判断是否设置了小数位,没有设置小数位，就不判断了
    ...
    range = function(el, type) {
        ...
        if(!valueMissing(el) && !isNaN(val)) {  //改过了
            if(type === "step") {
